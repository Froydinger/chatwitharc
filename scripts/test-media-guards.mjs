...
try {
  ...
  for (const model of ['gpt-image-1-mini','gpt-image-1','gpt-image-1.5-flash']) {
  ...
  assert.equal(ctx.fetches[0].body.model,'gpt-image-2.5-flare');
  ...
  for (const [version,prefs] of [[1,{model:'gpt-image-1-mini'}],[2,{quick:true}],[3,{model:'gpt-image-2'}],[4,{proImage:true}]]) {
