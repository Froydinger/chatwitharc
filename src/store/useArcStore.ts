...
export interface Message {
  ...
  webSources?: Array<{ url: string; title?: string; snippet?: string; content?: string }>; // Inline web citations
...
export interface ArcState {
  ...
  rightPanelTab: 'history' | 'media' | 'canvases' | 'apps' | 'quote' | 'links' | 'settings' | 'export';
  setRightPanelTab: (tab: 'history' | 'media' | 'canvases' | 'apps' | 'quote' | 'links' | 'settings' | 'export') => void;
  ...
  /**
   * What kind of long-form output the in-flight request resolved to, so the
        ...
        try {
          ...
          if (rpcError) {
            ...
            if (fallbackError) {
              ...
              const localOnly = (get().chatSessions || []).filter((s) => s.isLocalOnly);
              set({
                isOnline: false,
                syncedUserId: user.id,
                chatSessions: localOnly,
                messages: [],
