...
export function ChatInput({ onImagesChange }: { onImagesChange?: (hasImages: boolean) => void }) {
  ...
  // image-gen “active” (explicit or inferred)
  ...
  const cancelBanana = () => setForceImageMode(false);
  ...
  const handleSend = async () => {
    ...
    if (!imagesToProcess.length && userMessage) {
      isImageGenerationRequest = forceImageMode || checkForImageRequest(userMessage);
    }
    ...
    try {
      ...
      if (isUploadedImageEdit) {
        ...
        // generate flow with the simpler hidden prefix
    ...
    <div className="space-y-4">
      ...
      <div
        className={[
          ...
          shouldShowBanana ? "ring-2 ring-yellow-400/70 shadow-[0_0_40px_-6px_rgb(250_204_21_/_.6)]" : "ring-0",
        ].join(" ")}
      ...
      >
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              disabled={isLoading}
              className="shrink-0 h-12 w-12 rounded-xl flex items-center justify-center transition-colors duration-200 bg-muted/50 text-muted-foreground hover:bg-muted hover:text-foreground border border-border/40"
              aria-label="Image actions"
            >
              {/* exactly 20x20 to match lucide 5 sizing */}
        ...
        {/* active badge (with X to cancel); desktop shows label, mobile emoji only */}
        {shouldShowBanana && (
          <div className="shrink-0 h-8 pl-2 pr-1 sm:pl-3 sm:pr-2 rounded-full bg-yellow-400/20 border border-yellow-400/40 backdrop-blur-sm flex items-center gap-1 animate-pulse">
            <span className="text-[18px] leading-none">🍌</span>
            <span className="hidden sm:inline text-xs font-medium text-yellow-700 dark:text-yellow-400">
              Nano Banana
            </span>
            <button
              onClick={cancelBanana}
