...
export function AppBuilderWorkspace({ projectId: propProjectId, onClose, demo = false }: AppBuilderWorkspaceProps) {
  ...
  const lastUserRequest = [...messages].reverse().find(message => message.role === 'user')?.content.trim();
    ...
    <div className="flex h-full min-h-0 flex-col bg-[#0c0d0c]">
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
        <div className="flex-1 overflow-y-auto px-4 py-4">
          ...
          {(runView.error || runStatus === 'failed') && <div role="alert" className="rounded-xl border border-red-300/15 bg-red-200/[0.04] px-3 py-2 text-[10px] text-red-100/70"><p>{runView.error || 'This build did not finish. You can try the last request again.'}</p>{runView.error && ['queued', 'running'].includes(runStatus) && <Button size="sm" variant="outline" disabled={runView.busy} onClick={() => void appRunsRef.current?.reconnect().catch(() => {})} className="mt-2 h-7 border-white/10 px-2.5 text-[10px] text-white/75">Reconnect build</Button>}{runStatus === 'failed' && lastUserRequest && <Button size="sm" variant="outline" disabled={agentBusy || cloudActive} onClick={() => void handleSend(lastUserRequest, [])} className="mt-2 h-7 border-white/10 px-2.5 text-[10px] text-white/75">Try the last request again</Button>}</div>}
        </div>
