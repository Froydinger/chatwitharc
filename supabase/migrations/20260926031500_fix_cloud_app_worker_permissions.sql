-- Cloud app RPCs are SECURITY INVOKER and called only by the service-role
-- worker. Give that role only the publication-record operations its validated
-- build completion and publish steps need.
grant select, insert, update on public.cloud_app_publications to service_role;

-- Draft edits are already authorized by the user's App Builder submission.
-- Keep the exact lease, owner, Boost, receipt, and protected-path checks below;
-- publication remains explicitly approval-gated in cloud_app_publish_step.
...
begin
  ...
  if p_action='complete' then
    ...
    artifact := jsonb_build_object(
      ...
      'version',w.version,
      'title',p.title,
      'prompt',coalesce(p.prompt,p.title,'Arc App'),
      'ideTitle',p.title,
      'idePrompt',coalesce(p.prompt,p.title,'Arc App'),
      'fileCount',(select count(*) from jsonb_object_keys(v.files)),
      'url','/build/'||p.id::text,
      'published',exists(select 1 from public.cloud_app_publications where run_id=r.id and status='completed'),
      'publishedUrl',(select coalesce(url,netlify_url) from public.cloud_app_publications where run_id=r.id and status='completed' limit 1),
      'executed',false,
      'tested',false,
    ...
    if not public.complete_cloud_run(r.id,p_lease_token,
      ...
      coalesce(p_message,'{}') || jsonb_build_object('id','cloud-'||r.id::text,'role','assistant',
        'type','text','ideProjectId',p.id,'ideTitle',p.title,'idePrompt',coalesce(p.prompt,p.title,'Arc App'),'ideFileCount',(select count(*) from jsonb_object_keys(v.files)),
