-- Include a durable App Builder link and project metadata in Work results.
-- `url` always opens the saved builder project; `publishedUrl` is optional and
-- only present after a real, committed publication.
...
begin
  ...
  if p_action='complete' then
    ...
    artifact := jsonb_build_object(
      ...
      -- This is the durable Arc App Builder route. A separately published
      -- address is exposed as publishedUrl when publication really completed.
      'url','/build/'||p.id::text,
