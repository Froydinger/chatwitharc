-- Fix: public.cloud_image_step is SECURITY INVOKER and probed auth.users
-- directly. service_role has no SELECT on auth.users, so every 'begin'/'start'
-- returned 42501 -> PostgREST 403 -> a non-transient plain throw in
-- cloudImageStore. The worker's Promise.allSettled swallowed it, so each cloud
-- run holding a generate_image call sat on a 5-minute lease, was reclaimed, and
-- died at the attempt limit with no recorded error. No cloud_image_receipts row
-- has ever been written since 20260912102738 shipped.
--
-- Narrowest fix: move only the account probe into a SECURITY DEFINER helper.
-- The rest of cloud_image_step keeps its invoker posture over cloud_runs,
-- chat_sessions, image_generation_jobs and cloud_image_receipts.
begin;
set local lock_timeout = '2s';

create or replace function public.cloud_image_account_active(p_user_id uuid)
...
$fn$;
revoke all on function public.cloud_image_account_active(uuid) from public, anon, authenticated;
grant execute on function public.cloud_image_account_active(uuid) to service_role;
