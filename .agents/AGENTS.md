...
## Platform Environment & Deployments
* **No Lovable Connection**: The project is no longer connected to Lovable. Do NOT assume that pushing to Git automatically deploys backend schema changes, migrations, or Supabase Edge Functions. We can and should modify `supabase/functions/` and database files locally in this repository, and deployment of edge functions/migrations should be done using local tools (like `supabase-cli` if configured) or explicitly managed.
